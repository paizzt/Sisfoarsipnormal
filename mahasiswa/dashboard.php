<?php
session_start();
// Cek Login & Role Khusus Mahasiswa
...
if(isset($_POST['btn_update_profil'])){
    ...
    if(mysqli_num_rows($cek) > 0){
        $_SESSION['swal_icon'] = "error"; $_SESSION['swal_title'] = "Gagal"; $_SESSION['swal_text'] = "Email sudah digunakan.";
    } else {
        $update = mysqli_query($koneksi, "UPDATE users SET email='$email_baru' WHERE id_user='$id_user'");
        if($update){
            $_SESSION['email'] = $email_baru;
            $_SESSION['swal_icon'] = "success"; $_SESSION['swal_title'] = "Berhasil"; $_SESSION['swal_text'] = "Email diperbarui.";
...
// 1. Arsip Saya (File yang diupload user ini)
$q_my = mysqli_query($koneksi, "SELECT id_arsip FROM arsip WHERE pengunggah_id='$id_user' AND status_hapus='0'");
$jml_my = mysqli_num_rows($q_my);

// 2. Arsip Publik (Referensi belajar)
$q_pub = mysqli_query($koneksi, "SELECT id_arsip FROM arsip WHERE status_akses='publik' AND status_hapus='0'");
$jml_pub = mysqli_num_rows($q_pub);

// 3. Folder Tersedia (Kategori Publik)
$q_fol = mysqli_query($koneksi, "SELECT id_kategori FROM kategori WHERE status_folder='publik' AND status_hapus='0'");
$jml_fol = mysqli_num_rows($q_fol);
?>
...
<head>
    ...
    <title>Dashboard Mahasiswa - SISFO</title>
    ...
    <div class="wrapper">
        ...
        <nav id="sidebar" class="d-flex flex-column">
            ...
            <ul class="list-unstyled components flex-grow-1">
                ...
                <li>
                    <a href="upload.php" class="text-warning fw-bold"><i class="bi bi-cloud-arrow-up-fill"></i> Upload Arsip</a>
                </li>
                <li>
                    <a href="arsip_saya.php"><i class="bi bi-archive-fill"></i> Data Arsip</a>
                </li>
                <li>
                    <a href="cari_file.php"><i class="bi bi-search"></i> Cari File</a>
                </li>
            </ul>
        ...
        <div id="content">
            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm rounded-4 mb-4 px-4 py-2 sticky-top" style="z-index: 900;">
                <div class="d-flex align-items-center w-100">
                    ...
                    <div class="d-none d-md-block">
                        <h5 class="fw-bold mb-0 text-secondary" style="font-size: 1.1rem;">Dashboard Mahasiswa</h5>
                        <small class="text-muted">Halo, <?php echo $d_profil['nama_lengkap']; ?>!</small>
                    </div>
            ...
            <div class="container-fluid p-0">
                <div class="row g-4 mb-5">
                    <div class="col-xl-4 col-md-6">
                        <div class="card stat-card-modern grad-ocean shadow-sm">
                            <div class="card-body p-4">
                                <h5 class="mb-1 text-white-50">Arsip Saya</h5>
                                <h2 class="fw-bold mb-0"><?php echo $jml_my; ?></h2>
                                <p class="mb-0 small text-white-50 mt-1">Dokumen/Tugas yang Anda upload</p>
                                <i class="bi bi-cloud-arrow-up-fill bg-icon"></i>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-md-6">
                        <div class="card stat-card-modern grad-mint shadow-sm">
                            <div class="card-body p-4">
                                <h5 class="mb-1 text-white-50">Referensi Publik</h5>
                                <h2 class="fw-bold mb-0"><?php echo $jml_pub; ?></h2>
                                <p class="mb-0 small text-white-50 mt-1">Dokumen umum tersedia</p>
                    ...
                    <div class="col-xl-4 col-md-6">
                        <div class="card stat-card-modern grad-sunset shadow-sm">
                            <div class="card-body p-4">
                                <h5 class="mb-1 text-white-50">Folder Kategori</h5>
                                <h2 class="fw-bold mb-0"><?php echo $jml_fol; ?></h2>
                                <p class="mb-0 small text-white-50 mt-1">Total folder publik</p>
                ...
                <div class="alert alert-warning shadow-sm border-0 rounded-4 d-flex align-items-center mb-4 p-4 fade show" role="alert">
                    <i class="bi bi-exclamation-circle-fill fs-1 me-3 text-warning"></i>
                    <div>
                        <h5 class="alert-heading fw-bold mb-1">Upload Tugas atau Dokumen?</h5>
                        <p class="mb-0 small">Simpan dokumen akademik Anda secara aman. Anda bisa mengatur privasinya nanti.</p>
                    </div>
                    <a href="upload.php" class="btn btn-warning text-white fw-bold rounded-pill ms-auto shadow-sm px-4">
                        <i class="bi bi-upload me-2"></i> Upload Baru
                    </a>
                </div>

                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                        <h6 class="fw-bold m-0 text-secondary"><i class="bi bi-clock-history me-2"></i>Riwayat Upload Anda</h6>
                        <a href="arsip_saya.php" class="btn btn-sm btn-outline-primary rounded-pill px-3">Lihat Semua</a>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    <?php
                                    // Query Khusus: Hanya menampilkan apa yang diupload oleh user session ini
                                    ...
                                    if(mysqli_num_rows($recent) > 0){
                                        while($r = mysqli_fetch_array($recent)){
                                            $badge_akses = ($r['status_akses'] == 'publik') ? 'bg-success' : 'bg-danger';
                                    ?>
                                    <tr>
                                        <td class="ps-4">
                                        ...
                                        <td><span class="badge <?php echo $badge_akses; ?> rounded-pill text-uppercase" style="font-size: 0.65rem;"><?php echo $r['status_akses']; ?></span></td>
                                        <td class="text-muted small"><?php echo date('d/m/Y', strtotime($r['tgl_upload'])); ?></td>
                                    </tr>
                                    <?php } 
                                    } else { ?>
                                        <tr><td colspan="4" class="text-center py-4 text-muted fst-italic">Anda belum mengupload file apapun.</td></tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                        </div>
                ...
                <form action="" method="POST">
                    <div class="modal-body p-0">
                        <div class="bg-deep-aqua text-white text-center p-5 rounded-top-4 position-relative">
                            ...
                            <span class="badge bg-warning text-dark mt-2">MAHASISWA</span>
                        </div>
                        <div class="p-4">
